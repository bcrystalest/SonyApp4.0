...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        for (NSInteger index = 0; index < separateLabelCount; index ++) {
            ...
            separateLabel.text = @":";
            separateLabel.font = [UIFont systemFontOfSize:11];
...
//    NSString *nowtimeStr = [formatter stringFromDate:datenow];//----------将nsdate按formatter格式转成nsstring时间转时间戳的方法:
    ...
    NSInteger timestampForInterger = [timeSp integerValue];
    
    _timestamp = timestamp - timestampForInterger;//获取时间差值
    
    if (_timestamp != 0) {
        timer =[NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(timer:) userInfo:nil repeats:YES];
        [[NSRunLoop currentRunLoop] addTimer:timer forMode:UITrackingRunLoopMode];//倒计时在滑动时也需要继续倒数 
    }
}
...
//    NSLog(@"%zd日:%zd时:%zd分:%zd秒",day,hour,minute,second);
    ...
    self.hourLabel.font = [UIFont systemFontOfSize:11];
    self.minuesLabel.text = [NSString stringWithFormat:@"%zd分",minute];
    self.minuesLabel.font = [UIFont systemFontOfSize:11];
    self.secondsLabel.text = [NSString stringWithFormat:@"%zd秒",second];
    self.secondsLabel.font = [UIFont systemFontOfSize:11];
}
